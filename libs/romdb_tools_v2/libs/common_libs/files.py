...
class FilePath(object):
    ...
    def _get_size(self):
        ...
        return os.path.getsize(self.u_path)
    ...
    def content(self, pb_recursive=False, ps_type='all', ptu_exts=()):
        ...
        :param ps_type: Content elements to get: 'all' => everything, files and dirs; 'files' => just files; 'dirs' => just directories.

        :param ptu_exts: Tuple with extension of files you want to keep. e.g. (u'jpg', 'png'). Be careful when using it with directories. 
...
if __name__ == '__main__':
    if True:
        with BackReader(u'README.md') as o_my_file:
            for u_line in o_my_file:
                print u'>>%s<<' % repr(u_line)
                #break

    if False:
        o_my_file = BackReader(__file__)
        o_my_file.open()
        for u_line in o_my_file:
            print '    >>', u_line
        o_my_file.close()

    # print o_my_file._o_file

    if False:
        with codecs.open(__file__, 'r', 'utf8') as o_file:
